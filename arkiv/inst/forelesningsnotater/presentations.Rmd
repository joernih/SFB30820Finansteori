...
filp <- '/inst/forelesningsnotater/presentations.Rmd'
xaringan::inf_mr(paste0(rprojroot::find_rstudio_root_file(),filp))
rmarkdown::render(paste0(rprojroot::find_rstudio_root_file(),filp))
system(paste0('brave ',rprojroot::find_rstudio_root_file(),'/inst/forelesningsnotater/presentationsF.html'))
```
...
```{r echo=F}
#xaringanthemer::style_extra_css(css=list(".losning"=list(color='red',"font-size" = "90%")),outfile = stdout())
...
Eksamen avholdes den ~~12~~13.12.2023
...
Hvor *Risikopremien (risikokostnaden)* representerer den nye komponenten. I kapittel 2-3 i læreboka vises det at for et prosjekt vil denne komponenten være et produkt av en *pris (kostnad per risikokomponent)* og *kvantumkomponent (antall risikoenheter tilhørende prosjektet)*. 
...
X=9.17(1+0.02)=9.17\cdot (1.02)=9.35
\end{equation}
...
<br>**Svar:**<br>
\begin{equation*}
\text{I et investeringsprosjekt er tegnfølgen i kontantstrømmen (–,+,+,…,+)} \\
\text{Tegnfølgen i et finansieringsprosjekt er (+,–,–,…,–)}
\end{equation*}

1. Hva er forholdet mellom kapitalkostnad og nåverdi for et investeringsprosjekt kontra for et finansieringsprosjekt?
<br>**Svar:**<br>
\begin{equation*}
\text{For investeringsprosjekter faller nåverdien med økende kapitalkostnad.} \\
\text{For finansieringsprosjekter stiger den.}
\end{equation*}
1. Hva reflekterer kapitalkostnaden (diskonteringsrenten) i et finansieringsprosjekt?
<br>**Svar:**<br>
Kapitalkostnaden i et finansieringsprosjekt reflekterer den kostnaden du alternativt kunne ha skaffet gjeld eller egenkapital til.
Jo høyere denne alternativkostnaden er, desto mer verd er det finansieringsprosjektet du verdsetter gjennom nåverdiberegningen.
...
```{r, echo=F, warning=F}
dfp <- data.frame(pr=as.numeric(df_prosj_kontantstrøm[9,])[-1],ks=as.numeric(df_prosj_kontantstrøm[8,])[-1])
ggplot(dfp,aes(ks, pr)) + geom_col() + labs(x='Kontantstrøm', y='Sannsynlighet')
```
...
Selve porteføljeavkastningen ( $r_p$ ) uten skatt mellom to perioder ( $0$ og $T$ ) er gitt ved

$r_p=\frac{P_{T}+Div_{0,T}-P_{0}}{P_{0}}$
...
\begin{equation}
E(r_p) = \sum_{i=1}^{N}w_{i}E(X_{i})=w_{1}E(X_{1})+w_{2}E(X_{2})+...+w_{N}E(X_{N})
\end{equation}
...
**Svar metode 1**
\begin{equation*}
E(r_p)=0.3(1/3\cdot 3+ 1/3\cdot 5 +1/3\cdot 9) + \\
0.7(1/3\cdot 3+ 1/3\cdot 12 +1/3\cdot 11) = 7.76
\end{equation*}
**Svar metode 2**
\begin{equation*}
E(r_p)=(1/3)3+(1/3)9.9 +(1/3)10.4=1/3(3+9.9+10.4) = 7.76
\end{equation*}

<p>4.</p> Hva er svaret på spm. 3 dersom begge tilstander er like sannsynlige?
<br>**Svar:**<br> 
\begin{equation*}
...
knitr::include_graphics(paste0(rprojroot::find_rstudio_root_file(),'/inst/forelesningsnotater/illustrasjoner/oversiktsbildet.jpg'))
```
...
.panel[.panel-name[Figur 2.3 (replikasjon)]
```{r, echo=F}
...
\begin{equation}
Korr(r_a,r_b)=\frac{Kov(r_a,r_b)}{Std(r_a)Std(r_b)}
\end{equation}

- $Korr(r_a,r_b)=1$ (fullstendig avhengige)
- $Korr(r_a,r_b)=0$ (fullstendig uavhengige)
- $Korr(r_a,r_b)=-1$ (fullstendig motsatt avhengige)

Løser denne for $Kov(r_a,r_b)=Korr(r_a,r_b)Std(r_a)Std(r_b)$

Som gjør at vi kan skrive
\begin{equation}
Var(r_p)=w_1^2Var(r_1)+w^2_2Var(r_2)+2w_1w_2Korr(r_a,r_b)Std(r_a)Std(r_b)
\end{equation}
...
\begin{equation*}
Korr(r_a,r_b)=\frac{-0.0045}{0.036\cdot 0.125}=-1
\end{equation*}
...
.panel[.panel-name[Figur 2.4 (replikasjon)]
```{r, echo=F}
...
## Appendiks: Utledning av variansen av summen av to tilfeldige variabler $X$ og $Y$.

Vi ønsker å finne variansen til $Z$, som er gitt ved:
...
$$= Var(X) + 2E[(X - \mu_X)(Y - \mu_Y)] + Var(Y).$$

---

Nå, når vi fokuserer på leddet $2E[(X - \mu_X)(Y - \mu_Y)]$, som innebærer kryssproduktet mellom $X$ og $Y$, betegner vi det som kovariansen mellom $X$ og $Y$ ($Cov(X,Y)$):

$$Cov(X,Y) = E[(X - \mu_X)(Y - \mu_Y)].$$

Ved å sette kovariansleddet tilbake i ligningen for $Var(Z)$, får vi:

$$Var(Z) = Var(X) + 2Cov(X,Y) + Var(Y).$$

Dette er formelen for variansen av summen av to tilfeldige variabler $X$ og $Y$.
...
Dersom vi utnytter sammenhengen om at $Korr(r_a,r_b)=\frac{Kov(r_a,r_b)}{Std(r_a)Std(r_b)}$, kan vi også skrive dette som:
\begin{equation}
=w_a^2Var(r_a)+w^2_bVar(r_b)+w^2_cVar(r_c) + \\
...
- Legg merke til at første del av uttrykket for porteføljevariansbestår av $N$ ledd, mens siste består av 
  ( $N^2-N$ ) ledd
...
+(\frac{1}{N})^2(Kov(r_1,r_2)+Kov(r_1,r_3)+...+Kov(r_N,r_{N-1}))))
\end{equation}
...
\begin{equation}
\overline{Kov}=\frac{1}{(N^2-N)}(Kov(r_1,r_2)+Kov(r_1,r_3)+...+Kov(r_N,r_{N-1}))
\end{equation}
...
.panel[.panel-name[R-kode (ikke pensum)]
```{r}
kovo <- c(0.0229,0.1,0.3)
varo <- 0.21
plotwf <- kovo %>% purrr::map_dfr(function(x){
tportvar <- '(1/N)*varo + (1-1/N)*kovo'
N <- 1:60
#  df_n <- data.frame(ind=x)
df_n <- data.frame(ind=x,N=N,varp=eval(parse(text=tportvar),c(varo=varo,kovo=x,list(N=N))))
})
#tdf <- tidyr::pivot_longer(df_n,)
#plotwf
```
]
]]
...
Ved å utnytte sammenhengen om at $Korr(j,m)=\frac{Kov(j,m)}{Std(j)Std(m)}$, kan vi også uttrykke beta som 

\begin{equation}
\beta_j=\frac{Korr(r_j,r_m)Std(r_j)}{Std(r_m)}
\end{equation}
...
.panel[.panel-name[Tabell]
```{r, echo=F}
htmlTable(df_eks_2_6, header=c("Aksje","Forventet avkastning","Standard avvik", "Korrelasjonskoeffisient"))
```
]
.panel[.panel-name[R-kode (ikke pensum)]
```{r}
...
.panel[.panel-name[Plot av forskjellige porteføljesammensetninger]
```{r, echo=F}
...
Kombinerer uttrykket for forventet avkastning og standardavvik ved å sette inn for $1-w$, gjør at vi kan skrive
\begin{equation}
...
- Gitt at $w<0 \Rightarrow E(r_p) > E(r_m)$ 
...
- Vi har at investor A (stor grad av risikoaversjon) ønsker å plassere 80 prosent av formuen risikofritt
  ...
  -  $Var(r_p)=0.8^2\cdot 0 + 0.2^2\cdot 0.03^2 +2\cdot 0.0\cdot 0.3\cdot 0=0.00004$
...
- Vi har at investor B (liten grad av risikoaversjon) ønsker at forventet avkastning i porteføljen skal være på 8.5%
  ...
  -  $Var(r_p)=-0.5^2\cdot 0 + 1.5^2\cdot 0.03^2 \cdot 0.07 + 2 \cdot 0.0\cdot 0.03\cdot 0=0.02$
  -  $Std(r_p)=\sqrt{.02}  =0.045$

---

## Appendiks: Regneregler for forventninger og varians

### Forventninger

Forventningsverdi er lineær, noe som betyr at den følger egenskapene til lineær transformasjon for tilfeldige variabler:

$$
E[aX + b] = aE[X] + b \quad \text{for alle konstanter } a \text{ og } b
$$

Dette betyr at du kan trekke konstanter ut av forventningsverdien og fordele dem over tilfeldige variabler.

Forventningsverdien av en konstant er konstanten selv:

$$
E[c] = c \quad \text{for alle konstanter } c
$$

Forventningsverdien av summen av to tilfeldige variabler er lik summen av deres individuelle forventningsverdier:
...
Dette er en spesiell tilfelle av lineariteten av forventningsverdi.

---

### Varians
Variansen er ikke lineær, men den følger likevel noen regler for lineære transformasjoner av tilfeldige variabler:

$$
\text{Var}(aX + b) = a^2\text{Var}(X) \quad \text{for alle konstanter } a \text{ og } b
$$

Dette betyr at variansen av en konstant multiplisert med en tilfeldig variabel $X$ er lik kvadratet av konstanten multiplisert med variansen til $X$.


Variansen av summen av to tilfeldige variabler $X$ og $Y$ er lik summen av deres individuelle varianser, pluss to ganger kovariansen mellom dem (hvis de er avhengige):

$$
\text{Var}(X + Y) = \text{Var}(X) + \text{Var}(Y) + 2\text{Cov}(X, Y)
$$

Her representerer $\text{Cov}(X, Y)$ kovariansen mellom $X$ og $Y$. Hvis $X$ og $Y$ er uavhengige, er kovariansen null, og du får tilbake den enkle regelen at variansen av summen er lik summen av variansene.

Hvis $a$ er en konstant, er variansen av produktet $aX$ lik $a^2$ ganger variansen til $X$:
...
htmlTable(df_prosj_kontantstrøm_lik, header=c("","w_1=1/3","w_2=1/3","w_3=1/3"))
```
...
$$
k_E=r_f+\beta_E [E(r_m) - r_f ]=0.03+0.83[0.08 - 0.03] = 0.0715
$$
1. Kapitalkostnad for gjeld
$$
k_G=r_f+\beta_G [E(r_m) - r_f ] = 0.03+0.1[0.05]= 0.035 
$$
...
Som allerede omtalt under forelesning 3 (`r tema[3]`), ville den relevante risikoen (systematiske risikoen) kunne knyttes opp mot *både* en enkelt aksje eller et prosjekt, hvor beta ble beregnet på grunnlag av 
\begin{equation}
\beta_j=\frac{Std(r_j)Korr(r_j,r_m)}{Std(r_m)}
\end{equation}
...
1. Ved $\beta$ lavere på det nye prosjektet enn hva som gjelder for hele selskapet, gjennomgående for *få* investeringsprosjekter vil bli satt i gang
1. Ved høyere $\beta$ på det nye prosjektet enn hva som gjelder for hele selskapet, gjennomgående for *mange* investeringsprosjekter vil bli satt i gang
...
knitr::include_graphics("illustrasjoner/finansiering.png")
```
...
- Kapitalmarkedets *tre* hovedoppgaver
	- Samle og kanalisere kapital
	- Omfordele og spre risiko
	- Verdsette økonomisk virksomhet

- Finansielle instrumenter
	- Er en kontrakt som består av to parter
	  - Part B: Mottar kapital fra den andre parten 
	  - Part A: Tilfører kapital, samtidig med at vedkommende oppnår *visse rettigheter* ovenfor den andre parten
...
- Det offentlig kan forsøke å redusere disse imperfeksjonene i form av institusjoner som finanstilsynet og lover og regler
...
\begin{equation}
P_0=\sum_{t=1}^{5\cdot 2}\frac{10000\cdot 0.05/2}{(1+0.06/2)^{t}} + \frac{10000}{(1+0.03)^{5 \cdot 2}} = 2132.55+7440.94 = 9573.49
\end{equation}

```{r, eval=F, echo=F} 
(0.05*10000)*((1-(1/(0.03))^(10+1))/(1-(1/(0.03))))
# [1] 7440.939
10000/(1-(1/0.03))^10
# [1] 8.007468e-12
```
...
- **Beregning av implisitte renter:**
...
\begin{equation}
(1+_{o}r_{T})^T=(1+_{o}r_{1})(1+_{1}f_{2})...(1+_{T-1}f_{T})
\end{equation}

---
Sluttperioden for $t$ kan derfor skrives som
\begin{equation}
(1+_{o}r_{t})^t=(1+_{o}r_{1})(1+_{1}f_{2})...(1+_{t-1}f_{t})
\end{equation}
Mens $t-1$ som
\begin{equation}
(1+_{o}r_{t-1})^{t-1}=(1+_{o}r_{1})(1+_{1}f_{2})...(1+_{t-2}f_{t-1})
\end{equation}

Løser vi den øverste ligningen mhp. $(1+_{t-1}f_{t})$ gir oss
\begin{equation*}
(1+_{t-1}f_{t})=\frac{(1+_{o}r_{t})^t}{(1+_{o}r_{1})(1+_{1}f_{2})...(1+_{t-2}f_{t-1})
}
\end{equation*}

Fra ligningen under kan vi sette inn for $(1+_{o}r_{1})(1+_{1}f_{2})...(1+_{t-2}f_{t-1})$. Som forteller oss at **termin-/forwardrenten** for periode $t$ er bestemt ved 
...
**Eksempel 5.2:** Vi har tre statsobligasjoner med pålydende 1000,- og årlig kupongutbetalinger på 60 kroner. Videre har vi
```{r, echo=F}
...
Løser dette for inflasjonsforventingene 

\begin{equation}
_{t-1}j^R_t=\frac{_{t-1}f^N_t-_{t-1}f^R_t}{1+_{t-i}f_t^R}
\end{equation}
---

**Eksempel 5.3:** Fra eksempel 5.2 fant vi at de implisitte terminrenten for to og tre år var på hhv. 6.01 og 6.51 prosent. Vi tror nå at realrenten de nærmeste tre årene vil være på 3.17 prosent.

*Forventet inflasjonsprosenter for periode 2 sett fra periode 1*
\begin{equation*}
_{1}j^R_2=\frac{_{1}f^N_2-_{1}f^R{2}}{1+_{1}f_2^R}
=\frac{0.0601-0.0317}{1+0.0317}=0.0275
\end{equation*}

*Forventet inflasjonsprosenter for periode 3 sett fra periode 2*
\begin{equation*}
_{2}j^R_3=\frac{_{2}f^N_3-_{2}f_{2}^R}{1+_{2}f_3^R}
...
\begin{equation}
D=\frac{\sum_{t=1}^{T}\frac{tX}{(1+r)^t} + \frac{T\cdot M}{(1+r)^T}}{P_o}=\\ \left[1 \frac{NV(X_1)}{P_0}\right]+\left[2 \frac{NV(X_2)}{P_0}\right]+\left[3 \frac{NV(X_3)}{P_0}\right]...\left[T \frac{NV(X_T)+M}{P_0}\right]
\end{equation}
...
```{r, echo=F}
htmlTable(head(dur), header= c("Periode","Kontantstrømselement","Diskontering","NV","veid NV"))
```
...
- Markedsverdi (framtidig inntjening)
	- Aksjekursen gir uttrykk for nåverdien av framtidig forventet inntjening per aksje
...
- Markedsverdien må være på minst 300 mill
...
Ex-rights-kursen ( $P_x$ ) er bestemt som et veid snitt av rights-on-kursen ( $P_0$ ) og 
emisjonskursen ( $P_e$ )
...
Dersom vi definerer forholdet mellom antall gamle aksjer ( $m$ ) og nye ( $m$ ) emiterte aksjer $N=n/m$.
...
Eller, dersom vi benytter at $N=n/m=\frac{172476762}{94862219}=1.82$

$$
P_x=\frac{1.82\cdot 13.20+9.45}{1+1.82}=11.86935
$$

Verdien av tegningsretten $T_n$ vil derfor være

$$
T_n=\frac{13.20-9.45}{1.82+1}=1.33
$$
...
Felles for aksjesplitt og fondsemisjon er at *kontantstrømmseffekten* er null.
...
Alterntive navn er: *investeringsbeta, prosjektbeta, forretningsbeta,  eller driftsbeta*.
...
- Dersom vi løser uttrykket ovenfor for $\beta_E$ får vi
...
til_b <- data.frame(goe=goe_v, invb=inb_v) %>%
	...
	dplyr::mutate(akb=invb+(invb-gjb)*goe) 
...
- Arbitrasjestrage (kjøp undervurdert for samme risikoprofil):
	- Investering: Aksjer $0.1\cdot 400 og Obligasjoner $0.1\cdot 500$ som totalt koster 90
	- Mottar: $0.1\cdot OFR$
]
...
1. Kjøp deg inn i det underprisede selskapet. Porteføljen må da tilpasses slik at
	...
	- Gitt med gjeld i det overprisede selskapet, låner du privat for å få samme gjeldsgrad som i det overprisede selskapet og benytter dette beløpet til å kjøpe egenkapital i det underprisede selskapet.
...
Som vist vil arbitrasje føre til lik verdifastsette, men hvordan kan vi fastsette *tallverdien* som blir bestemt i markedet? Svar: Forutsetninger for verdsetting av selskaper i Miller & Modigliani: 
...
**Eksempel 7.5**: For Demo ASA er $\beta_G=0.20$ og $\beta_E=1.4$. Selskapet er finansiert med like mye gjeld som egenkapital, $w_G=w_E=0.50$.
Den risikofrie renten i markedet $r_f=0.03$, mens markedsporteføljen forventede avkastning $E(r_p)=0.08$.
...
\end{equation*}
\begin{equation*}
k_G=r_f+\beta_G \left[E(r_m)-r_f \right] \\ = 0.03 + 0.20(0.08-0.03)=0.04
\end{equation*}
...
## Appendiks: Utledning av formelen for summen av en uendlig kontantstrøm

Nåverdien av en enkelt fremtidig kontantstrøm kan finnes ved hjelp av formelen:
$$PV = \frac{C}{(1 + r)^n}$$

Hvor:

- $PV$  er nåverdien av kontantstrømmen
- $C$ er kontantstrømmen i fremtiden
- $r$ er avkastningskravet (diskonteringsrenten)
- $n$ er antall perioder (for eksempel år) i fremtiden

---

Hvis vi tenker på en evigvarende kontantstrøm der vi mottar $C$ hvert år, kan vi begynne med å beregne nåverdien for de første få årene:
...
Nåverdien av hele den evigvarende kontantstrømmen vil være summen av nåverdiene av alle disse individuelle kontantstrømmene. Matematisk kan vi skrive dette som en uendelig rekke:

$$ PV = C \times \left( \frac{1}{(1 + r)} + \frac{1}{(1 + r)^2} + \frac{1}{(1 + r)^3} + ... \right)$$

Dette er en geometrisk rekke hvor hver term er et tall ganger forrige term. For denne rekken, er ratioen mellom etterfølgende termer $\frac{1}{(1 + r)}$.

For en geometrisk rekke med ratio $|r|<1$ (i vårt tilfelle er dette $\frac{1}{(1 + r)}$), er summen av rekken gitt ved:

$$
S = \frac{a}{1 - r} 
$$

Hvor $a$ er den første termen i rekken. 

---

For vår kontantstrømrekke er $a = \frac{C}{(1 + r)}$ og $r=\frac{1}{(1 + r)}$.

Setter inn verdiene vi har:

$$ PV = \frac{\frac{C}{(1 + r)}}{1 - \frac{1}{(1 + r)}}$$

Forenkle videre:

$$ PV = \frac{C}{(1 + r) - 1} = \frac{C}{r}$$

Og det er hvordan vi kommer frem til formelen for en evigvarende kontantstrøm:
...
- Toleddsbeskatning
  - Selskapsskatt ( $s_S$ )
  - Investorskatt, dvs. kreditorer ( $s_G$ ) og aksjonærer ( $s_E$ )
...
#- $s_S=0.25$ 
#- $s_E=0.2875$ 
#- $s_K=0.25$ (for bedrifter) 
#(1-0.25)-(1-0.25)*(1-0.14)
#Løsning 2022:
#- $s_S=0.22$ 
#- $s_E=0.3784$ 
#- $s_K=0.22$ (for bedrifter) 
#(1-0.25)-(1-0.25)*(1-0.25)
```

---

### Imperfeksjoner som vi skal se på framover i dette kurset
- Skatt
...
- Driftsresultatet etter skatt (overskuddet før renter ved 100% egenkapitalfinansiering) er gitt ved
\begin{equation}
OFR=OFRS(1-s) \\
OFR=(SI-FK-VK-A)(1-s) 
\end{equation}
...
Dvs. uten gjeld, endres overskuddet (O) fra tabell 8.1 fra 525 til 600.
...
Eller
$$
800(1-0.25)+0.04\cdot2500\cdot 0.25 \\
=600+25\\
=625
$$
...
- Gitt at både selskapet og kreditor i full skatteposisjon - eier er et selskap
	- **Talleksempel:** $s_S=s_K=0.25$ og $s_E=0$
	- $Skattenøytralitet:$ $n^*=(1-s_K)-(1-s_S)(1-s_E)=(1-0.25)-(1-0.25)(1-0)$ = 0
	- $dvs. skattenøytralt$

- Gitt at både selskapet og kreditor i full skatteposisjon - eier er en person
	- **Talleksempel:** 	
	- $Skattenøytralitet:$ $n^*=(1-s_K)-(1-s_S)(1-s_E)=(1-0.25)-(1-0.25)(1-0.14)$ = 0.105
	- $dvs. gjeldsfavorieserende$

Det norske skattesystemet (basert på tall fra 2016) gir moderate incentiver til å finansiere med gjeld.
...
1. Hvis kreditorene beskattes lite (mye) i forhold til eierne, selskapet bør ha mest (minst) mulig gjeld
...
- *Agentteorien* bygger på forutsetning om asymmetrisk informasjon *samt* antar målkonflikter mellom selskapets interessenter
...
- Opphørsdividene kommer som et resultat av en beslutning fra selskapet om å nedlegge driften 
	- Beløpet $$\text{salg av eiendeler} - (\text{avviklingskostnader} + \text{gjeld})$$
...
- Hverken fondsemisjon eller aksjesplitter innebærer noen utbetaling fra selskapet til eierne (se forelesning 6 `r tema[6]`)
	- Vi betrakter derfor i dette kurset ikke dette som en dividendevariant
...
\begin{equation}
DPA_t = DPA_{t-1} + a[b(OPA_t)-DPA_{t-1}]
\end{equation}
...
2013: `r 100*0.3` $=100\cdot 0.3$
<br>
2014: `r 120*0.3*0.6 + nu-0.6*nu` $=120\cdot 0.3\cdot 0.6 + 30-0.6\cdot 30$
<br>
2015: `r 150*0.3*0.6 + en-0.6*en` $=150\cdot 0.3\cdot 0.6 + 33.6-0.6\cdot 33.6$
<br>
2016: `r 80*0.3*0.6 + to-0.6*to` $=80\cdot 0.3\cdot 0.6 + 40.44-0.6\cdot 40.44$
...
- $D_t + TB_t$ (Dividende pluss betaling for tilbakekjøpte aksjer) kontantstrømmen som eierne tar *ut* av selskapet
- $EM_t$ (Emisjonsbeløpet ved emisjons av aksjer) kontantstrømmen som eierne *tilfører* selskapet
...
1. Økt (redusert) uventet dividende vil medføre (lavere) høyere aksjekurs
```{r, echo=FALSE, out.width="40%", fig.cap="", fig.align="center"}
...
[Skatt på utbytte i Norge](https://www.ageras.no/blog/skatt-paa-utbytte)
...
### Eksempler på opsjoner til forskjellige eiendeler
...
- Kontraktstørrelse: *Én opsjon gir rett til 100 aksjer*
...
1. Øvre betingelse: $K_0 \leq A_0$ (ikke høyere enn aksjens verdi)
...
- Verdien av kjøpsopsjon avhenger av kontraktstiden: Lengre kontraktstid økt verdi [(lenke her for egenkskapene ved en random walk prosess modeller)](https://people.duke.edu/~rnau/411rand.htm)
...
\begin{equation*}
A_0-m\cdot K = 620 -2.61\cdot 37.62 = 521.80
\end{equation*}

*Kontroll 2: Sikringsporteføljens avkastning*
\begin{equation*}
r = \frac{527}{521.80}-1 = 0.01
\end{equation*}
