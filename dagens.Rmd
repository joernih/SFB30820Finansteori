...
*Forventet inflasjon periode 2*
\begin{equation*}
_{1}f^R_2=\frac{i-_{t-i}f^R_t}{1+_{t-i}f^R_t}=\frac{0.0601-0.0317}{1+0.0317}=0.0275
\end{equation*}

*Forventet inflasjon periode 3*
\begin{equation*}
_{2}f^R_3=\frac{i-_{t-i}f^R_t}{1+_{t-i}f^R_t}=\frac{0.0651-0.0317}{1+0.0317}=0.0324
\end{equation*}
...
durasjon_df <- function(r=0.0153,r_m=0.0175,n=2,T=10,M=1000){
	dur <- data.frame(per=seq(1,T*n)) %>% 
		dplyr::mutate(kont=(M*r_m)/n) %>%
		dplyr::mutate(disk=(1+r/n)^per) %>%
		dplyr::mutate(NV=kont/disk) %>%
		dplyr::mutate(wNV=per*NV)
}
```
]
...
htmlTable(head(dur), header=
	  c("Periode","Diskontering","Kontantstrømselement","NV","veid NV"))
```
]
.panel[.panel-name[Durasjon]

```{r}
durasjon <- (sum(dur$wNV[1:18])+16582.60)/1019.50
```
]
]
...
### Bokbasert kontra markedsbasert egenkapitial
...
- Hvor antall aksjer per tegningsrettighet bestemmes av forholdet mellom antall gamle og nye aksjer

---

- **Beregning av tegningsrettigheter:**

*Ex-rights-kursen* ( $P_x$ ) er bestemt som et veid gjennomsnitt av
