...
rmarkdown::render("arbeidskrav2021_F_L.Rmd")
system("brave arbeidskrav2021_F_L.html")
#servr::httw()
```
...
<div class="solution">
Med usikkerhet vil selve formen på nåverdiuttrykket være det samme som uten usikkerhet. Imidlertid vil 
beregningene som utføres i teller og never være forskjellige. Med usikkerhet erstattes i telleren en sikker kontantstrøm av den forventede kontantstrømmen.
I nevneren blir den risikofrie renten av risikojustert rente (kapitalkostnaden) som hensyntar både tidskostnaden og usikkerheten tilknyttet prosjektet.
</div>

(2) Hva er forskjellen mellom forventet og realiserte verdi av en kontantstrøm?
<div class="solution">
Den forventede kontantstrømmen gir oss forventningsverdien til en usikre kontanstrømmen *før* usikkerheten har blitt realisert. Den realiserte verdien gis ett av utfallene 
fra den usikre kontantstrømmen *etter* at usikkerheten har blitt realisert.
</div>
...
Den enkelte investors risikoaversjon avgjør bare hvor mye som totalt skal satses på den risikable komponenten framfor den risikofrie. Resultat oppstår fordi
...
<ol>
  ...
  <li>Forventet avkastning:<br>
  $E(r_p)$=0.6\cdot(0.11)+0.4\cdot (0.07) $=`r port_returns`$</li>
  <li>Varians:</li>
  Vi har først at
  $Kov(r_a,r_b)=0.5[(-0.1-0.11)(0.2-0.07)]+ 0.2[(0.5-0.11)(-0.3-0.07)] +0.3[(0.2-0.11)(0.1-0.07)]=`r cov_mat[1,2]`$
  Som forteller oss at <br>
  $Var(r_p)=0.4^2(0.0549)+0.6^2(0.0361)-2(0.4)(0.6)0.0417=`r port_risk_v`$</li> <br>
  <li>Standardavvik:</li>
...
For Ole betyr dette at 
<br>
$0.42=w\cdot 0.02 + (1-w)\cdot 0.04$ <br>
Som løst mhp. $w$ gir oss <br>
$w=-19$ og $1-w=1+19=20$<br>
Dette tilsvarer investeringsbeløp tilsvarerende $w=20\cdot 2.000.000=40.000.000$, hvor $38000000$ er som en følge av et risikofritt lån.
</div>
...
<div class="solution">
Vi forutsetter her (1) ingen kredittrestriksjoner og at (2) sparerenten er lik utlånsrenten. <br>
Ole kan forvente å få høyere avkastning enn Lise, men det er usikkerhet knyttet til Oles strategi i motsetning til Lise. Om ikke sannsynlig, er det i alle fall mulig at den realiserte verdien gir ham en lavere avastning enn Lise. 
</div>
...
$$
k_E=r_f+\beta_E[E(r_m) - rf]= 0.025+0.90[0.05] = 0.07
$$
(2): Kapitalkostnad for gjeld
$$
k_G=r_f+\beta_G[E(r_m) - rf] = 0.025+0.20[0.05]= 0.035 
$$
(3): Totalkapitalkostnad for selskapet
$$
...
$$
Med skatt (helt korrekt)
$$
NV =\frac{(1-0.15)100.000}{0.0539}=1576.994
$$
</div>
...
<div class="solution">
\begin{equation}
P_0= \frac{Mr_k/n}{(1+r/n)^{1}} + \frac{Mr_k/n}{(1+r/n)^{2}} + \frac{Mr_k/n}{(1+r/n)^{3}}+\frac{M}{(1+r/n)^3}= \\
\frac{350}{1+0.06}+\frac{350}{(1+0.06)^2}+\frac{350}{(1+0.06)^3}+\frac{5000}{(1+0.06)^3}=5133.651
\end{equation}
</div>
...
$$
1+_0r_2=(500/495)^(1/2)=1.005038
$$
og 
$$
1+_0r_3=(500/495)^(1/3)=1.003356
$$
*Terminrente periode 2*
$$
_{1}f_{2}=\frac{(1+_{0}r_{2})^{2}}{(1+_{0}r_{1})^{1}} - 1 = 
\frac{(1.005038)^2}{(1.015)^1}-1= -0.00981
$$
*Terminrente periode 3*
$$
_{2}f_{3}=\frac{(1+_{0}r_{3})^{3}}{(1+_{0}r_{2})^{2}} - 1
=\frac{(1.003356)^3}{(1.005038)^2}-1= -0.00167
$$
</div>
...
<div class="solution">
Dagens forventninger til framtidige korte renter er at de for to perioder framover vil synke til et neagativt nivå, for deretter i tredje periode hente seg noe opp igjen.
</div>
...
[Lenke til Google Spreadsheet løsningsforslag](https://docs.google.com/spreadsheets/d/1RBtpzzrAY5OIzlgBSLfzGvJeHmMJxMC_cEl3rOXV4m8/edit#gid=126699775)
