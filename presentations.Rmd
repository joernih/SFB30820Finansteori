...
Grunnleggende forutsetning: Ledelsen i bedriften har som oppgave å treffe beslutninger som makismerer verdien av egenkapitalen til eierne. 
...
# Bedriften
#kb <- data.frame(prob=c(0.30,0.70),beskr=c("Nedgangstid","Oppgangstid"),x=c(50,-30),y=c(5,12),z=c(9,11)) %>% tidyr::pivot_longer(c(x,y,z))
#ggplot(kb, aes(x = factor(beskr), y = value)) + geom_histogram()
```
...
- Udeversifiserte eiere: 
  - Ser forventet kontantstrømm fra det nye prosjektet i sammenheng med bedriftens allerede eksisterende kontantstrøm

- Veldiversifiserte eiere: 
  - Ser forventet kontantstrømm fra det nye prosjektet i sammenheng med porteføljens eksisterende kontantstrøm
...
Merk: Mens standardavviket gir oss samme benevning som forventet verdi, er benevningen til variansen vanskeligere å forholde seg til ("det kvadrerte til benevningen av standardavviket")
...
Fra Metode 1 oppgitt under første forelesning har vi at $E(r_a)=0.2\cdot 0.16+0.5\cdot 0.12+0.3\cdot 0.06=0.11$ og $E(r_b)=0.2\cdot 0.05+0.5\cdot 0.20+0.3\cdot 0.40=0.23$.
...
Risikoen til dette sammensate fondet C kan måles ved bruk av både metode 1 og 2.
...
**Øvelse:** Ved bruk av Excel, forsøk å repliker resultatende fra følgende tabell
...
df_eks_2_1_w <- df_eks_2_1 %>%
  ...
  dplyr::mutate(wvar=round(dev^2*prob, digits=7))
htmlTable(df_eks_2_1_w, header=c("Tilstand","Pr(s)","r_a(s)","r_b(s)","w1","w2","r_p(s)","Pr(s)r_p(s)","rp(s)-E(r_p)","[rp(s)-E(r_p)]^2"))
```

Hvor $E(r_p)=$ `r sum(df_eks_2_1_w$ev)` , $Var(r_p)=$ `r sum(df_eks_2_1_w$wvar)` og 
$Std(r_p)=$ `r sqrt(sum(df_eks_2_1_w$wvar))`
...
2. Risikokompensjonen for en enkeltaksje er ikke knyttet til aksjens standardavvik (totalrisiko). Relevant risiko må derfor måles på andre måter.
...
plotwf <- c(rn) %>% purrr::map_dfr(function(r,df=df_eks_2_1){
	w <- seq(0,1,0.01)
	v <- as.vector(df[,(2)])
	m <- as.matrix(df[,(3:4)])
...
.panel[.panel-name[R-kode (ikke pensum)]
```{r, echo=T, eval=T}
plotwf <- c(-1,0,0.90,1) %>% purrr::map_dfr(function(r,df=df_eks_2_1){
	w <- seq(0,1,0.01)
	v <- as.vector(df[,(2)])
	m <- as.matrix(df[,(3:4)])
	covall <- cov.wt(m,v,method='ML')
...
.panel[.panel-name[Figur]
```{r, echo=F}
ggr <- ggplot2::ggplot(data=plotwf,ggplot2::aes(x=stdavk,y=forvavk, group_by=corr)) + ggplot2::geom_point()
plotly::ggplotly(ggr)
```
]
]
